...
import cn.z.common.ex.TblNotFoundException;
...
public class TblRegister {
  ...
  private static String tablePrefix =DBConfig.tablePrefix;
  ...
  public static void addMap(Class<?> clazz) {
    ...
    for (Field f : clazz.getDeclaredFields()) {  //表的所有字段
      ...
      if ((joinOn = f.getDeclaredAnnotation(JoinOn.class)) != null) {  //字段上有@JoinOn
        ...
        /*//mmyy 19/5/25 16:01 表名可能错误bug
        String[] refTblInfo = joinOn.tbl().split("(?i)AS");
        String refTbl = !StringUtils.isEmpty(joinOn.tbl()) ? tablePrefix + refTblInfo[0].trim() : tablePrefix + f.getType().getSimpleName();
        String refCol = joinOn.onCol();*/
// ————————————————————修改19/5/25 16:01bug之后————————————————————
        String[] refTblInfo = joinOn.tbl().split("(?i)AS");
