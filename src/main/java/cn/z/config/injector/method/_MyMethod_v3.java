 ...
 */
public class _MyMethod_v3 extends AbstractMethod {
  ...
  public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
    ...
    for (Type t : mapperClass.getGenericInterfaces()) {
      ...
      if (t instanceof ParameterizedType) {
        clazz = (Class) ((ParameterizedType) t).getActualTypeArguments()[0];  // 获取第一个元素：Dept.class
      }else {
        continue;
      }
      ...
      for (Method method : methods) {
        ...
        for (Annotation anno : method.getDeclaredAnnotations()) {
          ...
          // String joinType = jas.joinType().getVal();
          // String[] strings = jas.value();
          // ArrUtil.outln(strings);
        }
      }
      ...
      for (Field f : clazz.getDeclaredFields()) {
        ...
        if ((fieldAno=f.getDeclaredAnnotation(JoinOn.class)) != null) {
          ...
          TblRegister.addMap(modelClass.getSimpleName(),f.getName(),refTbl,refCol);
