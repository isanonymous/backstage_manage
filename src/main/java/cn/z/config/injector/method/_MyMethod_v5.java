 ...
 */
public class _MyMethod_v5 extends AbstractMethod {
  ...
  public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
    ...
    Map<String,String> joinTypeMap = new LinkedHashMap<>();
    ...
    Method[] methods = mapperClass.getDeclaredMethods();  //如:selectList()...
      for (Method method : methods) {
        ...
        for (Annotation anno : method.getDeclaredAnnotations()) {
          ...
          JoinAndSelect jas;
          FromTbl ft;
          if ((ft = getAnnotation(anno, FromTbl.class)) != null) {
                ...
                for (Object obj : (Object[]) invoke) {  //{LIR}join
                  ...
                  if (StringUtils.isEmpty(tblName)) {
                    String getTblName= prevTbl != null?prevTbl:getTblName(modelClass, tableInfo);
                                        ...
                                        //     getTblName = prevTbl;
                                        // }else {
                                        //   getTblName = getTblName(modelClass, tableInfo);
                                        // }
                    //获取当前modelClass的表名, 通过表名再获取join的表
                    TblRegister.Tbl tblByFk = TblRegister.getTbl(getTblName).getTblByFk("");
        ...
        if (selectCol.length()>0) {
          ...
          // System.out.println(selectCol.toString());
          sql.append("\n from ");
